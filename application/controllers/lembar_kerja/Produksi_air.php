...
class Produksi_air extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($upk) {
            ...
            $data['title'] = 'RENCANA PRODUKSI DAN PENJUALAN AIR UPK '
                . strtoupper($nama_upk) . ' <br> TAHUN ANGGARAN ';
        } else {
            $result = $this->Model_produksi_air->getDataProduksiAir($tahun);
            $data['data_pendapatan_air'] = $result['data'];
        ...
        $this->load->view('lembar_kerja/produksi_air/view_produksi_air', $data);
