...
class Pendapatan_luar_usaha extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($upk) {
            $result = $this->Model_pendapatan_luar_usaha->getDataPendapatanLuarUsaha($tahun, $upk);
            $data['data_pendapatan_air'] = $result['data'];
            $nama_upk = $result['nama_upk'];
            $data['title'] = 'RENCANA PENDAPATAN DILUAR USAHA UPK '
                . strtoupper($nama_upk) . ' <br> TAHUN ANGGARAN ';
        } else {
            $result = $this->Model_pendapatan_luar_usaha->getDataPendapatanLuarUsaha($tahun);
            $data['data_pendapatan_air'] = $result['data'];
            $data['title'] = 'RENCANA PENDAPATAN DILUAR USAHA (KONSOLIDASI) <br> TAHUN ANGGARAN ';
        }

        $this->load->view('templates/header', $data);
        $this->load->view('templates/navbar');
        $this->load->view('templates/sidebar');
        $this->load->view('lembar_kerja/pendapatan_luar_usaha/view_pendapatan_luar_usaha', $data);
