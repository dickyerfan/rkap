...
class Investasi extends MY_Controller
{
    ...
    public function index()
    {
        ...
        if ($upk && isset($list_upk[$upk])) {
            $title = "RENCANA PENGELUARAN INVESTASI " . strtoupper($list_upk[$upk]) . "  <br> TAHUN ANGGARAN ";
        } else {
            $title = "RENCANA PENGELUARAN INVESTASI ( KONSOLIDASI ) <br> TAHUN ANGGARAN ";
        }

        $data['biaya'] = $this->Model_investasi->get_investasi($tahun, $upk);
        ...
        $this->load->view('lembar_kerja/arus_kas/investasi/view_investasi', $data);
    ...
    public function export_pdf()
    {
        ...
        if ($upk && isset($list_upk[$upk])) {
            ...
            $title = "RENCANA PENGELUARAN INVESTASI <br> TAHUN ANGGARAN ";
        }
        ...
        $this->pdf->generate('lembar_kerja/arus_kas/investasi/laporan_pdf', $data);
    }
        ...
        if ($this->input->post()) {
            ...
            foreach ($no_per_id as $key => $value) {
                ...
                foreach ($bulan_dipilih as $bulan) {
                    ...
                    $data[] = [
                        'cabang_id'    => $cabang_id[$key], // <-- PERBAIKAN DI SINI
                        'no_per_id'    => $value,
                        'uraian'       => $uraian[$key],
            ...
            if (empty($data)) {
                ...
                redirect('lembar_kerja/arus_kas/investasi/tambah');
                return;
            }

            $result = $this->Model_investasi->insert_or_update($data);
            ...
            redirect('lembar_kerja/arus_kas/investasi?upk=' . $upk_redirect . '&tahun_rkap=' . $tahun_rkap);
        } else {
            $data['title'] = 'Input Investasi';

            // Ambil data Akun
            $data['no_per_id'] = $this->db->like('kode', '31.', 'after')
            ...
            $data['satuan_list'] = ['unit', 'meter', 'buah', 'set', 'm2', 'm3', 'ls'];

            $this->load->view('templates/header', $data);
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('lembar_kerja/arus_kas/investasi/upload_investasi', $data);
            $this->load->view('templates/footer');
        }
    }
    ...
    public function edit($cabang_id, $no_per_id, $uraian_encoded)
    {
        ...
        // 🟢 2. Jika tidak ada POST (tampilkan form edit)
            ...
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('lembar_kerja/arus_kas/investasi/edit_investasi', $data);
            $this->load->view('templates/footer');
        }
    }
    ...
    public function generate()
    {
        ...
        $data = $this->Model_investasi->get_investasi($tahun, 'all');
        ...
        $this->session->set_flashdata(
            ...
            '<div class="alert alert-success alert-dismissible fade show" role="alert">
                <strong>Berhasil!</strong> Data Investasi berhasil digenerate ke Arus Kas.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>'
        );
        redirect('lembar_kerja/arus_kas/investasi');
    }
}
