...
class Beban_sumber extends MY_Controller
// class di ganti mengambil dari MY_Controller letaknya di application/core
{
    ...
    public function index()
    {
        ...
        if ($upk && isset($list_upk[$upk])) {
            $title = "BIAYA SUMBER " . strtoupper($list_upk[$upk]) . "  <br>  TAHUN ANGGARAN ";
        } else {
            $title = "BIAYA SUMBER ( KONSOLIDASI ) <br> TAHUN ANGGARAN ";
        }

        $data['biaya'] = $this->Model_beban->get_biaya_sumber($tahun, $upk);
        $data['title'] = $title;
        $data['upk'] = $upk;
        $data['tahun'] = $tahun;
        ...
        if ($this->session->userdata('level') == 'Admin') {
            ...
            $this->load->view('lembar_kerja/lr/beban_sumber/view_biaya_sumber', $data);
    ...
    public function export_pdf()
    {
        ...
        $this->pdf->filename = "Lap_beban_sumber_{$tahun}.pdf";

        // Generate dari view khusus PDF
        $this->pdf->generate('lembar_kerja/lr/beban_sumber/laporan_pdf', $data);
    }
    ...
    public function tambah()
    {
        ...
        if ($this->input->post()) {
            ...
            foreach ($no_per_id as $key => $value) {
                ...
                $status = (preg_match('/^91\.05(\.|$)/', $value)) ? 1 : 0;
            ...
            if (empty($data)) {
                ...
                redirect('lembar_kerja/lr/beban_sumber/tambah'); // sesuaikan redirect
                return;
            }

            $result = $this->Model_beban->insert_or_update_sumber($data);
            ...
            redirect('lembar_kerja/lr/beban_sumber?upk=' . $upk_redirect . '&tahun_rkap=' . $tahun_rkap);
        } else {
            $data['title'] = 'Input Beban Sumber';

            // Ambil data Akun
            $data['no_per_id'] = $this->db->like('kode', '91.', 'after')
            ...
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('lembar_kerja/lr/beban_sumber/upload_biaya_sumber', $data);
            $this->load->view('templates/footer');
        }
    }
    ...
    public function edit($encoded_key = null)
    {
        ...
        if ($this->input->post()) {
            ...
            <strong>Sukses!</strong>  data beban Sumber berhasil diperbarui.
            ...
            $data['title'] = 'Edit Beban Sumber';
            $data['data_edit'] = $data_edit;
            $data['mapping_upk'] = $mapping_upk;
            $data['no_per_id'] = $this->db->like('kode', '91.', 'after')->order_by('kode', 'ASC')->get('no_per')->result();
            $data['nama_bulan'] = [
    ...
    public function generate()
    {
        ...
        $this->db->like('no_per_id', '91', 'after');
        ...
        if (empty($biaya_data)) {
            $this->session->set_flashdata(
                ...
                <strong>Error!</strong> Tidak ada data biaya Sumber ditemukan untuk tahun ini.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>'
            );
            redirect('lembar_kerja/lr/beban_sumber');
            return;
        }

        // 🔹 Hapus data lama tahun tersebut untuk no_per_id yang dimulai 91*
        $this->db->where('YEAR(bulan)', (int)$tahun);
        $this->db->like('no_per_id', '91', 'after');
        ...
        if ($this->db->trans_status() === FALSE) {
            $this->session->set_flashdata(
                'info',
                '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                <strong>Error!</strong> Gagal menyimpan data biaya Sumber ke Laba Rugi.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>'
            );
        } else {
            $this->session->set_flashdata(
                ...
                <strong>Sukses!</strong> Semua data biaya Sumber berhasil digenerate ke Laba Rugi.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>'
            );
        }
    ...
    public function detail($kode_akun)
    {
        ...
        // Ambil nama akun dari tabel no_per
        ...
        $data['tahun'] = $tahun;
        $data['title'] = "DETAIL BIAYA SUMBER - " . strtoupper($nama_akun) . " <br> TAHUN ANGGARAN ";
        $data['mapping_upk'] = $mapping_upk;
        ...
        if ($this->session->userdata('level') == 'Admin') {
            ...
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('lembar_kerja/lr/beban_sumber/detail_biaya_sumber', $data);
            $this->load->view('templates/footer');
        } else {
            $this->load->view('templates/pengguna/header', $data);
            $this->load->view('templates/pengguna/navbar');
    ...
    public function export_pdf_detail($kode_akun)
    {
        ...
        $this->pdf->setPaper('Folio', 'landscape');
        $this->pdf->filename = "Lap_beban_sumber_detail_{$kode_akun}_{$tahun}.pdf";

        $this->pdf->generate('lembar_kerja/lr/beban_sumber/laporan_pdf_detail', $data);
    }
}
