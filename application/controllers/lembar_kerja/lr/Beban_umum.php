...
class Beban_umum extends MY_Controller
// class di ganti mengambil dari MY_Controller letaknya di application/core
{
    ...
    public function index()
    {
        ...
        if ($upk && isset($list_upk[$upk])) {
            $title = "BIAYA UMUM DAN ADMINISTRASI " . strtoupper($list_upk[$upk]) . " <br>   TAHUN ANGGARAN ";
        } else {
            $title = "BIAYA UMUM DAN ADMINISTRASI (KONSOLIDASI) <br> TAHUN ANGGARAN ";
        }

        $data['biaya'] = $this->Model_beban->get_biaya_umum($tahun, $upk);
        $data['title'] = $title;
        $data['upk'] = $upk;
        $data['tahun'] = $tahun;
        ...
        $this->load->view('lembar_kerja/lr/beban_umum/view_biaya_umum', $data);
    ...
    public function export_pdf()
    {
        ...
        $this->pdf->filename = "Lap_beban_umum_{$tahun}.pdf";

        // Generate dari view khusus PDF
        $this->pdf->generate('lembar_kerja/lr/beban_umum/laporan_pdf', $data);
    }
    ...
    public function tambah()
    {
        ...
        if ($tahun_rkap != $tahun_wajib) {
            ...
            redirect('lembar_kerja/lr/beban_umum?tahun_rkap=' . $tahun_wajib);
            return;
        }
        ...
        if ($this->input->post()) {
            ...
            foreach ($no_per_id as $key => $value) {
                ...
                $status = (preg_match('/^96\.09(\.|$)/', $value)) ? 1 : 0;
            ...
            if (empty($data)) {
                ...
                redirect('lembar_kerja/lr/beban_umum/tambah');
                return;
            }

            $result = $this->Model_beban->insert_or_update_umum($data);
            ...
            redirect('lembar_kerja/lr/beban_umum?upk=' . $upk_redirect . '&tahun_rkap=' . $tahun_rkap);
        } else {
            $data['title'] = 'Input Beban Umum dan Administrasi';

            // Ambil data Akun
            $data['no_per_id'] = $this->db->like('kode', '96.', 'after')
            ...
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('lembar_kerja/lr/beban_umum/upload_biaya_umum', $data);
            $this->load->view('templates/footer');
        }
    }
    ...
    public function edit($encoded_key = null)
    {
        ...
        if ($this->input->post()) {
            ...
            foreach ($bulan_sel as $i => $bulan_num) {
                ...
                // Tentukan status otomatis (96.09 -> status 1)
                $status = (preg_match('/^96\.09(\.|$)/', $new_no_per_id)) ? 1 : 0;
            ...
            <strong>Sukses!</strong>  data beban Umum & Administrasi berhasil diperbarui.
            ...
            $data['title'] = 'Edit Beban Umum & Administrasi';
            $data['data_edit'] = $data_edit;
            $data['mapping_upk'] = $mapping_upk;
            $data['no_per_id'] = $this->db->like('kode', '96.', 'after')->order_by('kode', 'ASC')->get('no_per')->result();
            $data['nama_bulan'] = [
            ...
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('lembar_kerja/lr/beban_umum/edit_biaya_umum', $data);
            $this->load->view('templates/footer');
        }
    }
    ...
    //     // 🔹 Hanya ambil akun yang diawali 96 (Biaya Umum & Administrasi)
    //     $this->db->like('no_per_id', '96', 'after');
    ...
    //             <strong>Error!</strong> Tidak ada data Biaya Umum & Administrasi ditemukan untuk tahun ini.
    //             <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    //         </div>'
    //         );
    ...
    //             <strong>Sukses!</strong> Semua data Biaya Umum & Administrasi berhasil digenerate ke Laba Rugi.
    //             <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    //         </div>'
    //         );
    //     }
    ...
    public function generate()
    {
        ...
        // 🔹 Hapus data lama tahun tersebut untuk no_per_id yang dimulai 96*
        $this->db->where('YEAR(bulan)', (int)$tahun);
        $this->db->like('no_per_id', '96', 'after');
        $this->db->delete('rkap_rekap');
        ...
        if ($this->db->trans_status() === FALSE) {
            $this->session->set_flashdata(
                'info',
                '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                <strong>Error!</strong> Gagal menyimpan data biaya Umum & Administrasi ke Laba Rugi.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>'
            );
        } else {
