...
// class di ganti mengambil dari MY_Controller letaknya di application/core
{
    ...
    public function edit($encoded_key = null)
    {
        ...
        if ($this->input->post()) {
            ...
            foreach ($bulan_sel as $i => $bulan_num) {
                ...
                // Tentukan status otomatis (93.04 -> status 1)
                $status = (preg_match('/^93\.04(\.|$)/', $new_no_per_id)) ? 1 : 0;
    ...
    public function detail($kode_akun)
    {
        ...
        $data['tahun'] = $tahun;
        $data['title'] = "DETAIL BIAYA TRANSMISI & DISTRIBUSI - " . strtoupper($nama_akun) . " <br> TAHUN ANGGARAN ";
        $data['mapping_upk'] = $mapping_upk;
        ...
        if ($this->session->userdata('level') == 'Admin') {
            ...
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('lembar_kerja/lr/beban_trandis/detail_biaya_trandis', $data);
            $this->load->view('templates/footer');
        } else {
            $this->load->view('templates/pengguna/header', $data);
            $this->load->view('templates/pengguna/navbar');
    ...
    public function export_pdf_detail($kode_akun)
    {
        ...
        $this->pdf->setPaper('Folio', 'landscape');
        $this->pdf->filename = "Lap_beban_trandis_detail_{$kode_akun}_{$tahun}.pdf";

        $this->pdf->generate('lembar_kerja/lr/beban_trandis/laporan_pdf_detail', $data);
    }
}
