                        ...
                        <div class="col-md-6">
                            ...
                            <div class="table-responsive" style="max-height: 300px; overflow-y: auto;">
                                <table class="table table-bordered table-striped table-sm">
                                    ...
                                    <tbody>
                                        <?php if (empty($evaluasi_upk_ini) && empty($evaluasi_upk_depan)) : ?>
                                            <tr>
                                        ...
                                        <?php else : ?>
                                            <?php
                                            $all_uraian = [];
                                            foreach ($evaluasi_upk_ini as $e) {
                                                $all_uraian[$e->uraian_evaluasi] = $e->satuan;
                                            }
                                            foreach ($evaluasi_upk_depan as $e) {
                                                $all_uraian[$e->uraian_evaluasi] = $e->satuan;
                                            }

                                            $evaluasi_ini_map = [];
                                            foreach ($evaluasi_upk_ini as $e) {
                                                $evaluasi_ini_map[$e->uraian_evaluasi] = $e->rkap;
                                            }
                                            $evaluasi_depan_map = [];
                                            foreach ($evaluasi_upk_depan as $e) {
                                                $evaluasi_depan_map[$e->uraian_evaluasi] = $e->rkap;
                                            }
                                            ?>
                                            <?php foreach ($all_uraian as $uraian => $satuan) : ?>
                                                <tr>
                                                    <td><?= $uraian ?></td>
                                                    <td class="text-center"><?= $satuan ?></td>
                                                    <td class="text-end"><?= isset($evaluasi_ini_map[$uraian]) ? number_format($evaluasi_ini_map[$uraian], 0, ',', '.') : '-' ?></td>
                                                    <td class="text-end"><?= isset($evaluasi_depan_map[$uraian]) ? number_format($evaluasi_depan_map[$uraian], 0, ',', '.') : '-' ?></td>
                                                </tr>
                                            <?php endforeach; ?>
                                        <?php endif; ?>
                                    </tbody>
