...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .section-title { font-size: 8pt; font-weight: bold; margin: 6px 0; }
...
<body>
    ...
    <main>
                ...
                ?>
                    ...
                    <?php foreach ($produk['tarif'] as $kategori => $data_tarif) : ?>
                        <tr>
                            ...
                            <td><?= number_format($data_tarif['persen'], 0, ',', '.') ?></td>
                            <?php
                            for ($i = 1; $i <= 12; $i++) :
        ...
        <p class="section-title">KELOMPOK HARGA</p>
        <table class="data-table">
            <thead>
                <tr>
                    <th>URAIAN</th>
            ...
            <tbody>
                <?php foreach ($pendapatan['harga'] as $produk) : ?>
                    <tr style="font-weight:bold;">
                        ...
                        <?php for ($i = 1; $i <= 12; $i++) : ?>
                            <td class="num"><?= number_format($produk['harga']); ?></td>
                        <?php endfor; ?>
                        <td class="num-bold"><?= number_format($produk['harga'] * 12); ?></td>
                    </tr>

                    <?php foreach ($produk['tarif'] as $tarif => $row) : ?>
                        <tr>
                            <td style="padding-left:16px;">- <?= ucfirst($tarif); ?></td>
                            <!-- <td class="text-center"><?= $row['persen']; ?>%</td> -->
                            <?php for ($i = 1; $i <= 12; $i++) : ?>
                                <td class="num"><?= number_format($row['harga']); ?></td>
                            <?php endfor; ?>
                            <td class="num-bold"><?= number_format($row['harga'] * 12); ?></td>
                        </tr>
                    <?php endforeach; ?>
                <?php endforeach; ?>
            </tbody>
        </table>
        <p class="section-title">PENDAPATAN OPERASIONAL AMDK (Rp)</p>
        <table class="data-table">
            <thead>
                <tr>
                    <th>URAIAN</th>
            ...
            <tbody>
                <?php foreach ($pendapatan['pendapatan'] as $produk) : ?>
                    <tr style="font-weight:bold;">
                        ...
                        $total = 0;
                        for ($i = 1; $i <= 12; $i++) {
                            echo '<td class="num">' . number_format($produk['pendapatan'][$i]) . '</td>';
                            $total += $produk['pendapatan'][$i];
                        }
                        ?>
                        <td class="num-bold"><?= number_format($total); ?></td>
                    </tr>

                    <?php foreach ($produk['tarif'] as $tarif => $row) : ?>
                        <tr>
                            <td style="padding-left:16px;">- <?= ucfirst($tarif); ?></td>
                            <td><?= number_format($row['persen'], 0); ?></td>
                            <?php
                            $subtotal = 0;
                            for ($i = 1; $i <= 12; $i++) {
                                echo '<td class="num">' . number_format($row['pendapatan'][$i]) . '</td>';
                                $subtotal += $row['pendapatan'][$i];
                            }
                            ?>
                            <td class="num-bold"><?= number_format($subtotal); ?></td>
                        </tr>
                    <?php endforeach; ?>
